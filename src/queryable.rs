...
pub trait Queryable: Sized + Sync + Serialize + for<'de> Deserialize<'de> {
    ...
    async fn find_by_id<T: EntityData>(
        ...
        let result = db.query_one(&query, vec![id.into()]).await?;
        match result {
    ...
    fn prepare<T: EntityData>(db: &impl RelationalDatabase) -> SqlExecutor<impl RelationalDatabase, T> {
