...
impl Dao<CartItem> for CartItemDao<CartItem> {
    ...
    fn entity_to_map(entity: &CartItem) -> Vec<(String, Value)> {
        ...
        map.push(("added_at".to_string(), Value::Text(entity.added_at.clone())));
        map
    }
...
impl Dao<Payment> for PaymentDao<Payment> {
    ...
    fn entity_to_map(entity: &Payment) -> Vec<(String, Value)> {
        ...
        map.push(("paid_at".to_string(), Value::Text(entity.paid_at.clone())));
        map
    }
...
fn test_payment_process() {
    ...
    assert_eq!(saved_payment.unwrap().order_id, order.id);
}
*/
// 测试库存更新
#[test]
